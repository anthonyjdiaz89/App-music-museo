...
import {
  ...
  getLocalVersion,
  syncLibrary,
} from "../../features/library/services/sync";
  ...
  const loadLastSyncDate = useCallback(async () => {
    try {
      ...
      if (timestamp) {
        setState((prev) => ({
          ...prev,
          lastSyncDate: new Date(parseInt(timestamp)),
        }));
      }
    } catch (e) {
  ...
  const shouldSync = useCallback(async (): Promise<boolean> => {
    ...
    if (lastSyncStr) {
      ...
      if (elapsed < MIN_SYNC_INTERVAL) {
        console.log(
          `[useCatalogSync] Última sincronización hace ${Math.floor(
            elapsed / 1000
          )}s, omitiendo`
        );
        return false;
    ...
    try {
      ...
      if (localVersion == null || remoteManifest.version > localVersion) {
        console.log(
          `[useCatalogSync] Nueva versión disponible: ${remoteManifest.version} > ${localVersion}`
        );
        return true;
      }
    ...
    const subscription = AppState.addEventListener(
      ...
      (nextAppState: AppStateStatus) => {
        if (
          ...
          console.log(
            "[useCatalogSync] App volvió al foreground, verificando sync"
          );
          syncCatalog(false);
        }
