    ...
    try {
        ...
        if (error) {
            ...
            if (error.message.includes('Bucket not found')) {
                throw new Error(`El bucket "${bucket}" no existe en tu proyecto de Supabase. Por favor créalo en el Dashboard > Storage.`);
            }
            // Mejora del mensaje de error para permisos (RLS)
            if (error.message.includes('row-level security policy')) {
                throw new Error(`Permiso denegado. Necesitas configurar una "Policy" en el bucket "${bucket}" que permita INSERT/UPDATE a usuarios públicos (anon).`);
            }
            throw error;
        }
        return data;
...
export async function updateCatalogVersion() {
    try {
        ...
        const { data, error } = await supabase
            .from('catalog_version')
            .upsert({ id: 1, version: newVersion, updated_at: new Date().toISOString() })
            .select()
            .single();
