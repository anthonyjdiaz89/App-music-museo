...
async function fileOk(localUri: string, expectedSize?: number | null) {
  ...
  try {
    ...
    if (expectedSize && info.size != null && info.size !== expectedSize)
      return false;
    return true;
  } catch {
    return false;
...
export async function syncLibrary(
  ...
  if (!manifest)
    throw new Error("No se pudo obtener el manifiesto de Supabase");

  onProgress &&
    onProgress(
      `Catálogo v${manifest.version} con ${manifest.items.length} pistas`
    );
  ...
  await FileSystem.writeAsStringAsync(
    LOCAL_LIBRARY_PATH,
    JSON.stringify({ version: manifest.version, items }, null, 2),
    { encoding: FileSystem.EncodingType.UTF8 }
  );
  await FileSystem.writeAsStringAsync(
    LOCAL_VERSION_PATH,
    JSON.stringify({ version: manifest.version, at: Date.now() }),
    { encoding: FileSystem.EncodingType.UTF8 }
  );
...
export async function loadLocalLibrary(): Promise<null | {
  version?: number;
  items: any[];
}> {
  if (Platform.OS === "web") {
  ...
  const ENABLE_REMOTE_SYNC =
    process.env.EXPO_PUBLIC_ENABLE_AUTO_SYNC === "true";
  ...
  try {
    ...
    const needsSeedRefresh =
      localVersion == null || localVersion < BUNDLED_LIBRARY_VERSION;

    if (needsSeedRefresh) {
      console.log(
        `[sync] Refrescando catálogo local desde bundle (v${BUNDLED_LIBRARY_VERSION})...`
      );
