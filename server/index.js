...
// --- SERVIR ARCHIVOS ESTÁTICOS Y ENRUTAMIENTO POR SUBDOMINIO ---
const DIST_ADMIN = path.join(ROOT, 'dist');
...
app.use((req, res, next) => {
  ...
  // Si es una petición API, dejarla pasar siempre (el router de API está definido abajo)
  if (isApi) return next();

  // Detectar subdominio 'admin'
  // Soporta: admin.localhost, admin.midominio.com, etc.
  if (host.startsWith('admin.')) {
    // --- LÓGICA PARA ADMIN PANEL ---
    if (fs.existsSync(DIST_ADMIN)) {
      // Servir estáticos del admin si el archivo existe
      const filePath = path.join(DIST_ADMIN, req.path);
      if (fs.existsSync(filePath) && fs.statSync(filePath).isFile()) {
        return res.sendFile(filePath);
      }
      // Si no es un archivo y no es API, servir index.html (SPA)
      return res.sendFile(path.join(DIST_ADMIN, 'index.html'));
    } else {
      return res.status(404).send('Admin build not found. Run npm run build:web in admin-panel.');
    }
  } else {
    // --- LÓGICA PARA APP PRINCIPAL / MÓVIL ---
    if (fs.existsSync(DIST_APP)) {
      // Servir estáticos de la app
      const filePath = path.join(DIST_APP, req.path);
      if (fs.existsSync(filePath) && fs.statSync(filePath).isFile()) {
        return res.sendFile(filePath);
      }
      // Fallback SPA para la app
      return res.sendFile(path.join(DIST_APP, 'index.html'));
    } else {
      // Si no hay build de app, mostrar mensaje simple
      return res.send('Museo del Vallenato - API Server Running. (App build not found)');
    }
  }
});
...
// ========== SUPABASE MCP SYNC ENDPOINTS ==========

/**
 * Get sync status between local and Supabase
...
app.post('/api/sync/push', async (req, res) => {
  try {
    const albums = await supabaseMCP.syncAlbumsToSupabase();
    const tracks = await supabaseMCP.syncTracksToSupabase();
    res.json({ ok: true, albums, tracks });
  } catch (e) {
    res.status(500).json({ error: String(e) });
  }
});
...
/**
 * Pull remote data from Supabase to local
 * @route POST /api/sync/pull
 */
app.post('/api/sync/pull', async (req, res) => {
  try {
    const albums = await supabaseMCP.pullAlbumsFromSupabase();
    const tracks = await supabaseMCP.pullTracksFromSupabase();
    res.json({ ok: true, albums, tracks });
  } catch (e) {
    res.status(500).json({ error: String(e) });
  }
...
app.post('/api/sync/full', async (req, res) => {
  try {
    const result = await supabaseMCP.fullSync();
 ...
 * @route POST /api/sync/realtime/enable
 */
app.post('/api/sync/realtime/enable', (req, res) => {
  try {
    const channels = supabaseMCP.setupRealtimeSync();
    res.json({ ok: true, message: 'Real-time sync enabled', channels: Object.keys(channels) });
  } catch (e) {
    res.status(500).json({ error: String(e) });
  }
});

// ========== END SUPABASE MCP ENDPOINTS ==========
...
app.get('/api/library', (req, res) => {
  try {
    const appLibraryPath = path.join(ROOT, 'assets', 'data', 'library.json');
